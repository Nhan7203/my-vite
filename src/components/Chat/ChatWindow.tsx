...
const ChatWindow = ({ onClose }: ChatWindowProps) => {
    ...
    const fetchChats = async (userId: number) => {
        try {
            const response = await fetch(`https://localhost:7030/api/Chat/getUserChat?userId=${userId}`);
            if (response.ok) {
    ...
    const handleUserClick = async (userId: number) => {
        ...
        try {
            const [currentUserChatsResponse, selectedUserChatsResponse] = await Promise.all([
                fetch(`https://localhost:7030/api/Chat/getUserChat?userId=${currentUserId}`),
                fetch(`https://localhost:7030/api/Chat/getUserChat?userId=${userId}`)
            ]);

            const currentUserChats = currentUserChatsResponse.ok ? await currentUserChatsResponse.json() : [];
            const selectedUserChats = selectedUserChatsResponse.ok ? await selectedUserChatsResponse.json() : [];
            ...
            const combinedMessages = [...filteredCurrentUserChats, ...filteredSelectedUserChats].sort(
                (a: ChatMessage, b: ChatMessage) => new Date(a.createdDate).getTime() - new Date(b.createdDate).getTime()
            );
                    ...
                    {messages.length === 0 ? (
                        ...
                        messages.map((message, index) => (
                            <Typography
                                ...
                                sx={{
                                    backgroundColor:
                                        message.senderId === currentUserId ? 'lightblue' : '#f0f0f0',
                                    padding: '8px',
                                    borderRadius: '8px',
                                    margin: '4px 0',
